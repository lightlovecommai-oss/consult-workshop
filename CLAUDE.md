...
## 🔴 開工前先讀這三份（2026-09-01 之後，本檔多處已過期）

| 讀什麼 | 為什麼 |
|---|---|
| **`_交接-2026-09-01-完整簡報.md`** ⭐ | **從這裡開始。** 不只是結論清單——含**三個問題的分析框架**（重點看到什麼／鉤子／留存）、每個模組的**問答與被否決的選項**、漏斗全景、四色對照、v2 元件判決表。<br>⚠️ **先讀 §2 分析框架**：遇到本檔沒涵蓋的情況，用那組問題自己推，推出來的會跟老師的直覺一致。**結論會過期，框架不會。** |
| **`demo.html`**（新） | 四模組的**互動原型＝規格**。一支手機從溝通健檢走到付款，可反覆跑。要改 app 先跑一次它 |
| **`../../5-企劃與產品手冊/productkit/1-手冊（內部）/30-App介面規範.md`**（新） | **螢幕介面唯一真相**：字體（介面一律無襯線）／字級四級／色層四階／圓角間距／ATPI 四色怎麼選／順序鎖 A→T→P→I／元件／數字露出政策／禁止清單 |

### ⚠️ 本檔已知過期的地方（2026-09-01）
...
**productkit 同一時間只讓一個 session 寫。** 大腦層的檔互相引用太密，兩邊同時寫必然出事。
執行專案（consult-workshop／comconverttest／LP）可以平行。
動 productkit 前先 `git -C <productkit> status --short | wc -l`，數字很大就先問誰在動。

**固定文案斷行規則（2026-09-05 老師抓包後定案）。** 手機自動換行會把詞組切碎——
實例：「端一塊牛肉出來——講**結果**，不講過程」被切成「……——講／結果，不講過程」，
動詞跟受詞拆行、語意直接模糊。規則：
1. 大字級固定句（練習主句、hero 句、標語）超過一行寬，一律**手動安排斷行點**（逗號或破折號處），不交給瀏覽器。
2. 資料層（judgement.js 等）用 `\n` 標斷行點；渲染層的 `md_()` 把 `\n` 轉 `<br>`。
3. 行尾不可留 1–2 字孤字；粗體詞組（`**X**`）不可跟前面的動詞拆行。
4. 新增／改文案後用手機寬度（~375px）檢查實際斷行。
