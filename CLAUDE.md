...
## 已知待處理
- 計分口徑：每維能力 = 投入%（任務累積，`Math.min(100)` 頂住）× 市場驗證係數（`validationFactor`，見計分常數 `TARGET_AMOUNT`/`TARGET_COUNT`/`VALID_FLOOR`）。投入的「整個營期固定滿分」(`DIMS[k].max`) 校準尚未定案；`TARGET_*` 目標值也待真實數據校準。設計全貌見專案記憶 scoring-model-design。
